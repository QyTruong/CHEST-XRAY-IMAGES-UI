...
def overlay_two_masks_on_image(image, mask_gt, mask_pred, alpha_gt=0.3, alpha_pred=0.5):
    overlay = image.copy()
    ...
    mask_gt_color = np.zeros_like(image)
    mask_gt_color[:, :, 0] = mask_gt_bin * 180  # Blue channel (dương)
    mask_gt_color[:, :, 1] = mask_gt_bin * 100  # Green channel nhẹ để thành xanh dương lợt

    mask_pred_color = np.zeros_like(image)
    mask_pred_color[:, :, 2] = mask_pred_bin * 255  # Red channel max (đỏ đậm)

    # Apply overlays
    overlay = cv2.addWeighted(overlay, 1, mask_gt_color, alpha_gt, 0)
    overlay = cv2.addWeighted(overlay, 1, mask_pred_color, alpha_pred, 0)
